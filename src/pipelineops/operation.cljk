(ns pipelineops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (PipelineTransportAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the PipelineTransportGovernor
  ...
  human pipeline-administration coordinator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected).

  ADMINISTRATIVE / LOGISTICS COORDINATION ONLY -- this graph never
  reaches a state that writes anything resembling a pipeline operating
  parameter (pressure, flow rate, valve, shutoff). Its only two
  terminal, SSoT-mutating nodes are `:commit` (writes an
  administrative/logistics coordination record + audit fact) and `:hold`
  (writes only a rejection audit fact)."
...
(defn build
  ...
  `pipelineops.store/Store`).
  opts:
    :advisor      -- a `pipelineops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PipelineTransportAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; PipelineTransportGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :segment-id (:segment-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :segment-id (:segment-id request) :by (:by approval)}]}
            {:disposition :hold
